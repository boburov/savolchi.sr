...
export class ChanelService {
  ...
  async create(
    ...
    if (adminHasChannel)
      throw new BadRequestException('Admin allaqachon kanal yaratgan');
    ...
    if (banner)
      bannerUrl = await this.bunny.uploadFile(banner, 'channel_banner');
  ...
  async getOne(id: string) {
    ...
    const chaneldata = await this.prisma.channel.findUnique({
      where: { id },
      include: {
        subject: {
          include: {
